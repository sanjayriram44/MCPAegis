...
def test_direct_db_and_eval_emit_w5():
    ...
    assert all(f.weakness_id == "W5" for f in findings)
    assert len(findings) == 2


def test_direct_file_and_net_emit_w6_w7():
    findings = from_sinks(
        [
    ...
    assert by_id["W6"].tool_name in {"read", "write"}
    assert any(f.weakness_id == "W6" for f in findings)
    assert by_id["W7"].tool_name == "fetch"
...
def test_collect_static_findings_includes_injection():
    ...
    assert any(r.weakness_id == "W5" for r in records)
